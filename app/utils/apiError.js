export function getApiError(error, customMessages = {}) {
    ...
    const overrides = typeof customMessages === 'string' 
        ? { default: customMessages } 
        : customMessages;

    // 1. Check validation errors (e.g. error.data.data.details, error.data.details, or error.data.errors)
    const details = error?.data?.data?.details || error?.data?.details || error?.data?.errors;
    ...
    const status = error?.data?.statusCode || error?.statusCode || error?.status || error?.response?.status;
